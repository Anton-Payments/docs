...
This walks you from an empty sandbox to a delivered payout. It takes about five minutes if you already have a sandbox API key.

<Info>
  Every request below uses the sandbox (`https://api.antonpayments.dev`) and a test key (`ak_test_...`). No real funds move. Switch to `https://api.antonpayments.com` and an `ak_live_*` key when you're ready for production — see [Going Live](/guides/going-live).
</Info>
...
- A sandbox API key from the [merchant dashboard](https://app.antonpayments.com). Sandbox access is available before KYB — you do not need an approved merchant account to finish this quickstart.
- `curl`, Node 18+, PHP 8+, or Go 1.21+ — any one is enough.

Store the key in an environment variable so the examples below work verbatim:

```bash
export ANTON_API_KEY=ak_test_your_sandbox_key_here
```
...
const response = await fetch("https://api.antonpayments.dev/v1/merchant", {
  headers: { Authorization: `Bearer ${process.env.ANTON_API_KEY}` },
});
...
```php PHP
$ch = curl_init("https://api.antonpayments.dev/v1/merchant");
curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_HTTPHEADER => ["Authorization: Bearer " . getenv("ANTON_API_KEY")],
]);

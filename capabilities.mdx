...
| Webhooks, documents, RFIs, onboarding, users & settings, API credential management, audit log, reference data       | Any authenticated credential — available on every tier. Responding to an RFI is dashboard-only and needs `rfis.respond` (Admin or Operator role). |

Two classes of operation are **dashboard-only** regardless of scope, and reject machine credentials with `403 oauth_client_not_accepted` or `403 insufficient_permissions`:

- **Approval decisions** — payout approve/reject in the maker-checker flow requires a signed-in human, because the approver ≠ creator guard is meaningless for a credential.
- **Detokenized PII readback** — beneficiary PII retrieval, verification name reveal, and captured-document viewing happen only in the dashboard, admin-role-gated and audit-logged per access. Machine credentials can _write_ PII (`PUT /v1/beneficiaries/{id}/pii` — your system is its source) but never read it back, so a leaked credential cannot harvest identities.

...
Each environment is completely isolated. API keys, payees, payouts, balances, and webhook subscriptions created in one environment do not exist in the other.
...
<Warning>
  **Production payouts are irreversible** once they reach `sent` status. Verify payee details and amounts before submitting. Use [idempotency keys](/concepts/idempotency) on every request to prevent duplicates.
</Warning>

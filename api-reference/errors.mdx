...
| `503 Service Unavailable`   | A downstream dependency (rail provider, FX provider, secure vault, identity provider) is degraded or circuit-broken. Retry with backoff.                                                                           |
...
| ----- | --------------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `403` | `capability_required`       | `this endpoint requires the "payouts" account capability`                     | The merchant account does not hold the capability this endpoint requires — typical for an [AI-compliance-only account](/capabilities) calling a payout-surface endpoint. Effective access is always `scopes ∩ capabilities`, and the capability gate is checked first. | Not retryable. Contact Anton to change the account's capability tier.                                                                                                   |
| `403` | `insufficient_scope`        | `this endpoint requires a credential with the "payouts" scope`                | The account holds the capability, but the calling credential was created without the matching scope.                                                                                                                                                                   | Not retryable with this credential. Use or create a credential carrying the required scope, then retry. See [Capabilities & Scopes](/capabilities).                     |
| `403` | `insufficient_permissions`  | `insufficient permissions`                                                    | For portal users: the caller's role does not grant the required permission. For API credentials: the credential's scopes do not map to the permission this endpoint requires (e.g. PII readback is never machine-grantable).                                           | Portal: have a user with the right role act, or adjust team roles. API: check your credential's scopes; if the operation is dashboard-only by design, perform it there. |
| `403` | `oauth_client_not_accepted` | `this endpoint is only available through the merchant dashboard`              | An OAuth+DPoP credential called a dashboard-only endpoint — approval decisions, PII reveal, credential/team management, and similar human-only surfaces reject machine callers outright.                                                                               | Not retryable with any API credential. Perform the action in the merchant dashboard.                                                                                    |
| `403` | `api_key_not_accepted`      | `this endpoint is only available through the merchant dashboard`              | A legacy static API key called a dashboard-only endpoint. (Static keys are no longer accepted on v1 at all — see `401 static_api_keys_disabled`.)                                                                                                                      | Migrate to OAuth 2.0 + DPoP; dashboard-only operations remain dashboard-only.                                                                                           |
...
| `409` | `idempotency_in_flight`   | A prior request with the same `Idempotency-Key` is still being processed.                                    | Wait briefly and retry with the **same** key — once the first attempt settles, the retry replays its response. |
| `503` | `idempotency_unavailable` | The idempotency subsystem is temporarily unavailable. Anton fails closed rather than risk double-processing. | Retry with backoff, same key.                                                                                  |
...
| `422` | `payout_not_cancellable` | The payout's current state does not allow cancellation (already submitted to a rail, completed, `cannot_be_processed`, etc.).                 | Re-fetch the payout; cancellation is only valid before submission.                                                                               |
| `409` | `payout_on_hold`         | The payout is under review (`on_hold: true`) and cannot be cancelled until the review concludes. The response gives no reason.               | Wait for the next `payout.*` webhook, then retry if the payout is still cancellable.                                                             |
...
A payout that fails **after** creation (during asynchronous rail submission) transitions to `failed` and records a sanitized failure on its event timeline. Read it from `GET /v1/payouts/{id}/events` — the terminal event's `metadata` carries:

| Metadata key                          | Contents                                                                                                                                              |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `failure_code`                        | Stable machine-readable code (table below). Branch on this, not on `reason`.                                                                          |
| `failure_category`                    | `validation`, `vault`, or `provider` — coarse grouping for dashboards and triage.                                                                     |
| `reason`                              | Short human-readable message. Safe to log and display; never contains internal error chains.                                                          |
| `rail_error_code`                     | Present on provider-originated failures: the canonical rail classification, e.g. `rail_provider_rejected`, `rail_insufficient_funds`, `rail_timeout`. |
| `rail_provider`, `rail_provider_code` | Present on provider-originated failures: which provider refused and its native code, where available.                                                 |

| `failure_code`                 | Category     | Meaning                                                                                                         | Remediation                                                                                                  |
| ------------------------------ | ------------ | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `beneficiary_not_found`        | `validation` | The beneficiary record could not be loaded at submission time.                                                  | Verify the beneficiary exists and is active, then create a new payout.                                       |
| `vault_unavailable`            | `vault`      | The credential vault was temporarily unavailable; reserved funds were released.                                 | Transient — create a new payout (fresh `Idempotency-Key`) after a short backoff.                             |
| `detokenization_failed`        | `vault`      | Payment credentials or beneficiary details could not be retrieved from the vault; reserved funds were released. | Retry later; if persistent, re-create the instrument or contact support with the payout ID.                  |
| `provider_registration_failed` | `provider`   | The beneficiary could not be registered with the payment provider.                                              | Check the beneficiary/instrument details for the destination corridor; contact support if they look correct. |
| `provider_unavailable`         | `provider`   | The provider was unreachable, rate-limited, or timed out.                                                       | Transient — create a new payout after a backoff.                                                             |
| `provider_rejected`            | `provider`   | The provider refused the payment instruction. Terminal for this payout.                                         | Inspect `reason` and `rail_provider_code`; fix the beneficiary/instrument data before re-submitting.         |
| `insufficient_funds`           | `provider`   | The provider reported insufficient funds for the instruction.                                                   | Reserved funds are released back to your balance. Retry later or contact support.                            |

On every terminal failure the full reserved amount (source + fees) is released back to your available balance, and a `payout.failed` webhook fires. The webhook payload carries the payout resource; fetch the events endpoint for the `failure_code`.

### Intelligence

The strict Intelligence request contract returns two codes beyond the standard set. See [Intelligence — Rate Limits and Errors](/guides/intelligence/rate-limits-errors).

| HTTP  | `code`          | Meaning                                                                                              | Remediation                                                          |
| ----- | --------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| `400` | `unknown_field` | The request contains a field outside the strict contract (unknown fields are rejected, not ignored). | Remove the field; pin to `contract_version: intelligence-public-v1`. |
| `400` | `type_mismatch` | A field has the wrong JSON type.                                                                     | Fix the type.                                                        |
...
<Note>
  Uploads larger than the 32 MB limit currently surface as `400 file_too_large`
  (the BodySizeLimit middleware rejects oversize requests before the handler
  runs). The catalog entry in the [Payload size](#payload-size) section reflects
  the intended long-term shape (`413 file_too_large`); an internal fix is
  tracked to align the status.
</Note>
...
| `422` | `beneficiary_create_failed`     | Catch-all for creation failures (vault tokenization, database). The detailed cause is logged server-side. | Retry; share the `X-Request-ID` if persistent.                 |
| `422` | `beneficiary_update_failed`     | Generic update failure.                                                                                          | Inspect the message; retry.                                    |
| `422` | `beneficiary_pii_update_failed` | PII update in the secure vault failed.                                                                              | Retry; if persistent, share the `X-Request-ID`.                |
...
| `422` | `instrument_create_failed` | Catch-all creation failure (vault tokenization, database).                                             | Retry; share the `X-Request-ID` if persistent.  |
...
| `403 capability_required`, `403 insufficient_scope`                                                                                                                                                       | No             | Account-tier or credential-scope gate. Fails identically until the account capability or credential changes. See [Capabilities & Scopes](/capabilities). |
| `409 idempotency_conflict`                                                                                                                                                                                | No             | You sent a new payload under a used key. Generate a new key.                                                                                             |
| `409 idempotency_in_flight`                                                                                                                                                                               | Yes (same key) | The original attempt is still processing. Wait briefly, retry with the same key to replay its response.                                                  |
...
<Warning>
  When retrying a mutating request (`POST`, `PUT`, `PATCH`), always send the
  same `Idempotency-Key` you used on the original attempt. Without it, the
  server cannot distinguish a retry from a new request, and you risk
  double-processing. See [Idempotency](./idempotency).
</Warning>
...
res = requests.post(
    ...
    headers={
        # dpop_proof is signed per request and binds this method + URL.
        # See /authentication for how to generate it.
...
<Note>
  ...
  `X-Request-ID` header value from the response. Anton retains a full
  server-side trace keyed by that ID and can diagnose without asking you to
  reproduce.
</Note>

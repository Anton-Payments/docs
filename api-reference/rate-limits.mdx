...
| `/v1/intelligence/*` (authenticated) | 120 / minute per merchant, on top of the global quota |
| `/v1/intelligence/*` (anonymous public test surface, where enabled) | 120 / minute per IP |
...
**Intelligence** runs under a dedicated per-merchant limiter in addition to the global quota, because evaluations fan out to multiple scoring systems per call. The limit applies across all `/v1/intelligence/*` endpoints. Sustained-volume screening integrations can request a raise the same way as the global ceiling.

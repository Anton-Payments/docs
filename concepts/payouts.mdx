...
| `sent` | Rail provider confirmed dispatch to the payee's bank/wallet | No |
...
1. **Validation** — Amount, currency, and payee are validated
...
7. **Delivery** — Funds are dispatched to the payee
...
```bash
curl https://api.antonpayments.dev/v1/payouts/pay_abc123/cancel \
  -X POST \
  -H "Authorization: Bearer ak_test_..."
```

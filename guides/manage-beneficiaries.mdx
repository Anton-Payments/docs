...
A beneficiary is the identity of a payout recipient. Every beneficiary belongs to exactly one merchant — two merchants paying the same real person each create their own beneficiary record. PII submitted at creation is tokenized in a secure vault; Anton's core database stores only a display name and country plus an opaque token reference.
...
Vault tokens are immutable. Updating PII replaces the entire token — you send the full detail block, Anton re-tokenizes, and the old token is dropped:
...
<Warning>
  Beneficiaries that cannot be used for payouts (status: `blocked`) cannot be
  restored via the API. Contact support if you believe this is in error.
</Warning>
...
- `beneficiary.blocked` (the beneficiary can no longer be used for payouts)

...
- `payout.screening_failed` — payout on hold until it is reviewed
- `payout.cannot_be_processed` — payout cannot be processed
...
- `beneficiary.blocked` — beneficiary can no longer be used for payouts

Requests for Information:
- `rfi.created` — Anton raised a Request for Information

`screening.hit` is no longer sent. A payout under review shows `on_hold: true` and its status instead.

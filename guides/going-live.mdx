...
Swap `ak_test_*` for `ak_live_*` in your production secrets manager and redeploy. Verify the first live request with a **very small payout** to a beneficiary you control — confirm the full lifecycle (`created → approved → sent → completed`) before ramping traffic.

...
<Steps>
  ...
  <Step title="Graceful handling of payouts under review">
    Payouts under review (`on_hold: true`) can clear in minutes, or take longer if an RFI is raised. Surface a pending state to your end user; do not treat review as an error. Handle `cannot_be_processed` as a final state.
  </Step>
</Steps>
...
<Steps>
  <Step title="Monitoring and alerting">
    Alert on: unexpected payout failures, webhook delivery retries exhausted, elevated `5xx` rates from the Anton API, payouts on hold for longer than you expect, and open RFIs older than your SLA.
  </Step>
